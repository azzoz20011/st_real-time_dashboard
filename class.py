...
st.set_page_config(
    page_title="Real-time S3 Image Predictor",
    layout="wide"
)
...
# Main app function
def main():
    st.title("Real-time S3 Image Predictor")
    
    # Sidebar for configuration
    with st.sidebar:
        st.header("Configuration")
        bucket_name = st.text_input("S3 Bucket Name", "my-gpr-dataa")
        image_prefix = st.text_input("Image Prefix", "scans/")
        refresh_interval = st.slider("Refresh Interval (seconds)", 1, 60, 5)
        
        st.header("Model Configuration")
        model_path = st.text_input("Local Model Path", "models/logistic_model.joblib")
        model_s3_key = st.text_input("Model S3 Key (Optional)", "models/logistic_model.joblib")
        classes_input = st.text_input("Classes (comma-separated)", "fruit,nothing")
        classes = [c.strip() for c in classes_input.split(",")]
        
        # Download model button
        if st.button("Download Model from S3"):
                ...
                if st.session_state['last_modified'] != last_modified:
                    ...
                    **File Information:**
                    ...
                    if os.path.exists(model_path):
                        prediction, confidence, probabilities = predict_image(image, model_path, classes)
