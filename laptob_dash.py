...
import numpy as np
import joblib
from sklearn.linear_model import LogisticRegression  # Changed from SVM to LogisticRegression
...
# Function to train or load logistic regression model
def train_or_load_model(model_path, classes):
    if os.path.exists(model_path):
        try:
            # Try to load the existing model
            model = joblib.load(model_path)
            # Check if it's a LogisticRegression model
            if not isinstance(model, LogisticRegression):
                st.warning("Existing model is not a LogisticRegression model. Creating a new one.")
                return create_model(model_path, classes)
            return model
        except Exception as e:
...
# Function to create a new logistic regression model
def create_model(model_path, classes):
    ...
    st.info(f"Created new logistic regression model and saved to {model_path}")
    
    return model

# Function to predict image class with logistic regression
def predict_image(image, model_path, classes):
    try:
        # Load or create the logistic regression model
        model = train_or_load_model(model_path, classes)
        ...
        # Resize to match the expected input size (same as before)
        target_size = int(np.sqrt(4096))  # ~45.25, we'll use 45
        ...
        elif len(features) < 4096:
            # Pad with zeros if needed
            features = np.pad(features, (0, 4096 - len(features)))
        ...
        # We can't fit the scaler with just one sample, so we just transform
        # In a real scenario, the scaler would be fitted on training data and saved
        features_scaled = features
        
        # Make prediction
        if hasattr(model, 'predict') and hasattr(model, 'predict_proba'):
            prediction = model.predict(features_scaled)[0]
            predicted_class = classes[prediction] if isinstance(prediction, (int, np.integer)) else prediction
            
            # Get prediction probabilities
            probs = model.predict_proba(features_scaled)[0]
            probabilities = {cls: float(probs[i]) for i, cls in enumerate(model.classes_)}
            
            # Find confidence of the predicted class
            ...
            st.warning("Model does not support predict or predict_proba methods.")
            return "Unknown", None, {"Unknown": 1.0}
    
    except Exception as e:
        st.error(f"Error during prediction: {str(e)}")
        return "Error", None, {"Error": 1.0}
    ...
    try:
        ...
        if probabilities:
            for cls, prob in probabilities.items():
                # Make sure column names are valid - replace any problematic characters
                col_name = f'prob_{cls}'.replace(' ', '_').replace('-', '_')
                new_result[col_name] = [prob]
        ...
        try:
            ...
            # Check if we need to add new columns for new classes
...
def main():
    ...
    with st.sidebar:
        ...
        model_path = "models/logistic_3class.joblib"  # Updated model name
        model_s3_key = st.text_input("Model S3 Key (Optional)", "models/logistic_3class.joblib")  # Updated default
        classes_input = "nothing,Pipe,Foil"  # Same three classes as required
    ...
    if model_s3_key and not os.path.exists(model_path):
        ...
        # Even if the file exists, verify it's a logistic regression model
        train_or_load_model(model_path, classes)
        ...
        if image_content is not None:
            try:
                ...
                if st.session_state['last_modified'] != last_modified:
                    ...
                    display_plotly_visualization(img_array_2d, plotly_placeholder, prediction, None)
                    ...
                    if prediction is not None and os.path.exists(model_path):
                        ...
                        **Prediction Results:**
                        ...
                        if confidence is not None:
                            ...
                            names = [cls for cls in classes]
                            prediction_text += "**Class Probabilities:**\n"
                            for cls, prob in probabilities.items():
                    
                                prediction_text += f"- {names[cls]}: {prob:.4f}\n"
